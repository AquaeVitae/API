...
from aquaevitae_api.mail.messages import (
    REQUEST_STATUS_UPDATE_SUBJECT,
    REQUEST_STATUS_DENIED_MESSAGE,
    REQUEST_STATUS_APPROVED_MESSAGE,
    REQUEST_CREATED_ADMIN_SUBJECT,
    REQUEST_CREATED_ADMIN_MESSAGE,
    REQUEST_CREATED_MESSAGE,
    REQUEST_CREATED_SUBJECT,
    REQUEST_DENIED_BY_SERVER_MESSAGE,
)
...
def notify_partnership_stakeholders(
    sender, instance, created, update_fields, *args, **kwargs
):
    if not created:
            ...
            match instance.status:
                case RequestStatusChoices.DENIED:
                    asyncio.run(
                        async_send_mail(
                            ...
                            message=REQUEST_STATUS_DENIED_MESSAGE
                ...
                case RequestStatusChoices.APPROVED:
                    asyncio.run(
                        async_send_mail(
                            subject=REQUEST_STATUS_UPDATE_SUBJECT,
                            message=REQUEST_STATUS_APPROVED_MESSAGE
                            % instance.agent_fullname,
                            recipient_list=[instance.agent_email],
                            fail_silently=True,
                            from_email=None,
        ...
        if instance.status == RequestStatusChoices.DENIED_BY_SERVER:
            asyncio.run(
                async_send_mail(
                    ...
                    message=REQUEST_DENIED_BY_SERVER_MESSAGE
                    % (instance.agent_fullname, instance.id),
                    recipient_list=[instance.agent_email],
                    fail_silently=True,
                    from_email=None,
                )
            ...
            admin_addresses = set(
                User.objects.distinct("email").values_list("email", flat=True)
            )
            admin_email = (
