...
class CreatePartnershipSerializer(CountryFieldMixin, serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        closed = q_partnerships.filter(~Q(status=RequestStatusChoices.WAITING), updated_at__gte=timezone.now()-timedelta(days=settings.REQUEST_TIME_LIMIT)).first()

        if waiting:
            attrs["status"] = RequestStatusChoices.DENIED_BY_SERVER
            attrs["comments"] = _("Already have an OPEN request for this company created by %s of ID %s\n") % (waiting.agent_email, waiting.id)
        elif closed:
            attrs["status"] = RequestStatusChoices.DENIED_BY_SERVER
            attrs["comments"] = _("There is a recent CLOSED request for this company created by %s of ID %s\n") % (closed.agent_email, closed.id)
