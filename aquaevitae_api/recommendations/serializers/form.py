...
class CreateFormSerializer(serializers.ModelSerializer):
    ...
    skin_types = serializers.MultipleChoiceField(choices=FormSkinTypeChoices.choices, required=True, allow_empty=False, validators=[lambda a: validate_max_length(a, settings.SKIN_TYPE_MAX_NUMBER)])
    skin_diseases = CreateFormSkinDiseasesSerializer(many=True, validators = [lambda a: validate_max_length(a, settings.SKIN_DISEASE_MAX_NUMBER)])
    ...
    def create(self, validated_data):
        ...
        for i in range(0, skin_types_len if skin_types_len > skin_diseases_len else skin_diseases_len):
            if i < skin_types_len:
