...
from products.constants import CATEGORY_CHOICES, PRODUCT_TYPE_CHOICES, SKIN_NEEDS_CHOICES, SIZE_TYPE_CHOICES, SOLAR_CARES_CHOICES
...
class ProductsAdmin(admin.ModelAdmin):
    list_display = ["id", "name", "type", "category", "link_company", "created_at", "available"]
    search_fields = ["id", "name", "company__name", "ingredients__name"]
    list_filter = [DeletedListFilter, CompaniesListFilter, CategoryListFilter, TypeListFilter, SkinNeedsListFilter, SkinTypeListFilter, SkinSolarNeedsListFilter]
    exclude = ["is_deleted", ]
    fields = [
        "id",
    ...
    ordering = ["-created_at"]
    inlines = [
        IngredientsInline, SkinTypeInline, SkinNeedsInline, SkinSolarNeedsInline
    ]
    ...
    def image_tag(self, obj):
        return format_html('<img src="{}" style="height: 200px"/>'.format( "/v1" + obj.image.url))
